...
//-- Alumno ----------------------------------------------------------------//
app.get('/estudiantes/alumno/:id', function(req, res){
	for( var i=ListaEstudiantes.length-1; i>=0; i-- )
		if( ListaEstudiantes[i].legajo == req.params.id ) break;
		if( i>=0 ) {
...
app.post('/estudiantes/alumno/:id', function(req, res){
	let p = ListaEstudiantes.some(function(p){ return p.legajo == req.params.id });
	if( p ) {
				//res.json({error: 'Ya existe ese legajo!'});
				res.status(400).send(req.body);
	} else {		
				let estudiante = new Estudiante (req.params.id, req.query.nombre, req.query.edad, req.query.carrera );
...
app.put('/estudiantes/alumno/:id', function(req, res){
	var i;
	for( var i=ListaEstudiantes.length-1; i>=0; i-- )
		if( ListaEstudiantes[i].legajo == req.params.id ) break;
		if( i>=0 ) {
...
app.delete('/estudiantes/alumno/:id', function(req, res){
	var i;
	for( var i=ListaEstudiantes.length-1; i>=0; i-- )
		if( ListaEstudiantes[i].legajo == req.params.id ) break;
		if( i>=0 ) {
...
//-- Inscripcion ---------------------------------------------------------//
app.get('/estudiantes/cursada/:id', function(req, res){
	var i;
	var ListaMaterias = [];
	let p = ListaEstudiantes.some(function(p){ return p.legajo == req.params.id });
	if( p ) {
				for( var i=ListaCursadas.length-1; i>=0; i-- )
					if( ListaCursadas[i].legajo == req.params.id ) {
...
app.post('/estudiantes/cursada/:id', function(req, res){
	var i;
	let p = ListaEstudiantes.some(function(p){ return p.legajo == req.params.id });
	if( p ) {
				for( var i=ListaCursadas.length-1; i>=0; i-- )
					if( ListaCursadas[i].legajo == req.params.id && ListaCursadas[i].turno == req.query.turno && ListaCursadas[i].dia == req.query.dia ) break;
					if( i>=0 ) {
									res.json({error: 'Ya esta inscripto ese dia en ese turno'});
					} else {
								let cursada = new Cursada (req.params.id, req.query.materia, req.query.turno, req.query.dia );
...
app.delete('/estudiantes/cursada/:id', function(req, res){
	var i;
	let p = ListaEstudiantes.some(function(p){ return p.legajo == req.params.id });
	if( p ) {
				for( var i=ListaCursadas.length-1; i>=0; i-- )
...
//-- Pago --------------------------------------------------------------------------//
app.get('/estudiantes/pago/:id', function(req, res){
	var PagosEstudiante = [];
	let p = ListaEstudiantes.some(function(p){ return p.legajo == req.params.id });
	if( p ) {
				for( var i=ListaPagos.length-1; i>=0; i-- )
					if( ListaPagos[i].legajo == req.params.id ) {
...
app.post('/estudiantes/pago/:id', function(req, res){
	var i;
	let p = ListaEstudiantes.some(function(p){ return p.legajo == req.params.id });
	if( p ) {
				for( var i=ListaPagos.length-1; i>=0; i-- )
					if( ListaPagos[i].legajo == req.params.id && ListaPagos[i].concepto == req.query.concepto && ListaPagos[i].cuota == req.query.cuota) break;
					if( i>=0 ) {
									//res.json({error: 'El pago del concepto ya esta imputado'});
									res.status(400).send(req.body);
					} else {
								let pago = new Pago (req.params.id, req.query.fecha, req.query.concepto, req.query.monto, req.query.cuota );
...
//-- Saldo ------------------------------------------------------- //
app.get('/estudiantes/saldo/:id', function(req, res){
	var i;
	var PagosEstudiante = [];
	let p = ListaEstudiantes.some(function(p){ return p.legajo == req.params.id });
	if( p ) {
				for( var i=0; i<=ListaArancel.length-1; i++ )
					if( ListaArancel[i].legajo == req.params.id && ListaArancel[i].estado == 'impago') {
